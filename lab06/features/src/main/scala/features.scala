...
object features extends App{
  lazy val spark: SparkSession = SparkSession.builder().appName("mikhail.pykhtin_features").getOrCreate()
  ...
  val users_items_df = spark.read
    .option("header", "false")
    .option("path", "/user/mikhail.pykhtin/users-items/20200429")
    .load()
  ...
  final_df
    ...
    .format("parquet")
    .mode("overwrite")
    .option("path", "/user/mikhail.pykhtin/features")
    .save()
